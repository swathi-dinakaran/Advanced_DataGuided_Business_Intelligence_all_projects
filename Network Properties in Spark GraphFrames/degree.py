...
def simple(g):
	...
	# YOUR CODE HERE
    extracted_edges=g.edges.rdd;
    flipped_edges= extracted_edges.map(lambda x: (x[1],x[0]))
	...
	# YOUR CODE HERE
    combined_edges= extracted_edges.union(flipped_edges).distinct()
    filtered_edges= combined_edges.filter(lambda x: x[1]!=x[0])
    edges= sqlContext.createDataFrame(filtered_edges , ['src','dst']).distinct()
     
    vertices = edges.selectExpr('src as id').unionAll(edges.selectExpr('dst as id')).distinct()
    
    new_graph= GraphFrame(vertices,edges)
    print(new_graph)
    return new_graph
